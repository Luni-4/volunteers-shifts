...
use crate::{AppState, APP_TITLE, COOKIE_ROUTE, POLICY_COOKIE};
...
async fn hide_index_cookie_dialog(jar: &CookieJar<'_>) -> Redirect {
    ...
    Redirect::to(uri!(crate::login::authentication::show_authentication))
}
...
#[get("/")]
async fn show_cookie_policy(state: &State<AppState>) -> Template {
    Template::render(
        "cookie_policy",
        context! {
            title: APP_TITLE,
